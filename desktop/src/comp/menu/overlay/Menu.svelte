...
<div class="relative h-full cursor-default">
  <button on:click={() => {isMenuOpen.set(!get(isMenuOpen)); openMenu.set(title)}}
    ...
    {#if isSettings}
      <svg xmlns="http://www.w3.org/2000/svg" class="icon-settings ml-auto" width="22" height="22" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  ...
  {#if $isMenuOpen && $openMenu === title}
    <div in:flyIn class="bg flex flex-col absolute top-0 z-50 min-w-[9rem] rounded-md mt-7 text-sm p-1 shadow-md w-48 -translate-x-0.5" class:right-0={isSettings}>
      {#each items as item}
        {#if item.divider}
          <MenuDiv/>
