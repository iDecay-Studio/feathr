...
<div class="menu relative h-full cursor-default {isSettings ? 'ml-auto pr-0.5' : ''}">
  <button on:click={() => {isMenuOpen.set(!get(isMenuOpen)); openMenu.set(title)}}
    ...
    {#if isSettings}
      <svg xmlns="http://www.w3.org/2000/svg" class="icon-settings size-5.5" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  ...
  {#if $isMenuOpen && $openMenu === title}
    <div in:flyIn class="menu-overlay bg flex flex-col absolute top-0 z-50 rounded-md mt-7 text-md p-1 shadow-md -translate-x-0.5 {isSettings ? 'right-0 w-34' : 'w-50 min-w-[9rem]'}">
      {#each items as item}
        {#if item.divider}
          <MenuDiv/>
