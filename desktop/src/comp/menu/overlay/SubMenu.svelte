...
<button class="relative w-full group data-[disabled]:opacity-50 data-[disabled]:pointer-events-none" data-disabled={!item.submenu.length ? "" : undefined}>
  ...
  <div data-submenu="" class="submenu absolute top-0 left-0 invisible duration-200 ease-out translate-x-full opacity-0 {isSettings ? 'ml-1 group-hover:ml-0' : 'right-0 mr-1 group-hover:mr-0'} group-hover:visible group-hover:opacity-100" class:hidden={!item.submenu.length}>
    <div class="bg z-50 min-w-[9rem] overflow-hidden rounded-md p-1 -mt-[5px] shadow-md animate-in {item.wide ? 'w-48' : 'w-32'} {isSettings ? 'absolute left-[unset] right-0 slide-in-from-right-1' : 'slide-in-from-left-1'}">
      <slot/>
    </div>
  </div>
</button>