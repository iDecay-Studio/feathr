...
>
  ...
  <div class="flex gap-1 items-center flex-wrap">
    ...
    <input id="inputReplace" class:hidden={!$replace}>
  </div>
  ...
  <div class="flex gap-1 items-center flex-wrap">
    ...
    {/if}
    <div id="count"/>
    <button id="btnPrev" class="btn p-0" on:click={() => app.editor.highlighter.prev()}>◀</button>
    <button id="btnNext" class="btn p-0" on:click={() => app.editor.highlighter.next()}>▶</button>
    <button id="btnReplaceAll" class="btn clear" on:click={() => app.cmdBar.runCmd(true)}>{$_('cmdBar.replace_all')}</button>
    <button id="btnClear" class="btn clear" on:click={() => app.cmdBar.clear()}>{$_('cmdBar.clear')}</button>
  </div>
</div>